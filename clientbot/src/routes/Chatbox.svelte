<script lang="ts">
    ...
    const addMsg = async () => {
        ...
        messenger.update((messenger : any) => [...messenger, message]);
        socket.send(message)
        await tick();
        ...
        <div class="flex flex-row items-center h-16 rounded-xl bg-white w-full px-4">
          ...
          <div class="flex-grow">
            <div class="relative w-full">
                <input bind:value={msg} on:keydown={e => e.key === 'Enter' && msg!=="" } type="text" class="flex w-full border rounded-xl focus:outline-none focus:border-indigo-300 pl-4 pr-10 h-10">
